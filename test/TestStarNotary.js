...
it('can change star name', async () => {
    ...
    let starName = await instance.starName.call(); // Calling the starName property
    assert.equal(starName, "Awesome Udacity Star"); // Assert if the starName property was initialized correctly
    await instance.changeName("Star 1", {from: owner});
    let newStarName = await instance.starName.call(); // Calling the starName property
    assert.equal(newStarName, "Star 1"); // Assert if the starName property was initialized correctly
 });